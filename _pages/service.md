---
layout: archive
title: ""
permalink: /service/
author_profile: true
---

## Mentoring
___
<ul class="spaced">
  <li>
    Varsha Rajkumar MS CS @ BU Spring 2026 (Next: Ph.D. Student, MPI-SWS)
    <ul>
      <li>Topic: Fault Detection in the Quest-V Hypervisor</li>
    </ul>
  </li>
</ul>

## Academic Service
___
<ul class="spaced">

  <li>
    Secondary Review:
    <ul>
      <li>Software: Practice and Experience (2025)</li>
      <li>IEEE Real-Time and Embedded Technology and Applications Symposium (RTAS)
        2025</li>
      <li>IEEE Real-Time Systems Symposium (RTSS) 2024</li>
    </ul>
  </li>

  <li>
    Artifact Evaluation:
    <ul>
      <li>USENIX Symposium on Operating Systems Design and Implementation (OSDI) 2026</li>
      <li>IEEE Real-Time and Embedded Technology and Applications Symposium (RTAS) (Secondary) 2026</li>
    </ul>
  </li>

  <li>
    Shadow Technical Program Committee:
    <ul>
      <li>Euromicro Conference on Real-Time Systems (ECRTS) 2025, 2026</li>
    </ul>
  </li>

</ul>

## Talks and Presentations
___
* <a target="_blank" rel="noopener noreferrer"
  href="https://sites.bu.edu/quest/">Getting Started with the Quest RTOS and
  Quest-V Partitioning Hypervisor</a> - RTSS 2024, ESWEEK 2025, RTSS 2025